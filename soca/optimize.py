...
def find_optimal_assignment(
    ...
    num_assigned = len(all_assigned)
    candidates = candidates_expertise[~candidates_expertise['Nom complet'].isin(all_assigned)]
    available_count = len(candidates)
    
    # Print funnel view - TUI-aware
    if tui_log:
        tui_log.info(f"{position_spec.position_name.upper()} | total={total_in_db} | unassigned={total_in_db - num_assigned} | expert={expertise_count} | avail={available_count}")
        ...
        logger.info("  %s | total=%d | unassigned=%d | expert=%d | avail=%d",
                position_spec.position_name.upper(), total_in_db, total_in_db - num_assigned,
...
def _calculate_candidate_score(
    ...
    # Ensure height is a float to avoid TypeError during comparison
    try:
        height = float(candidate['Alçada (cm)'])
    except ValueError:
        raise ValueError(f"Invalid height value: '{candidate['Alçada (cm)']}' from candidate '{candidate['Nom complet']}'")
